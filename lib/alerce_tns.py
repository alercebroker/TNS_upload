...
#    def get_ned(self, oid):
#        
#        # get ra, dec
#        if oid != self.oid:
#            self.get_stats(oid)
#            
#        co = coordinates.SkyCoord(ra=self.meanra, dec=self.meandec, unit=(u.deg, u.deg), frame='fk5')
#        Ned_table = Ned.query_region(co, radius=0.01 * u.deg, equinox='J2000.0')
#        if Ned_table is not None:
#            if len(Ned_table["Object Name"]) > 0:
#                return Ned_table
#            else:
#                return False
#        else:
...
#
#        co = coordinates.SkyCoord(ra=self.meanra, dec=self.meandec, unit=(u.deg, u.deg), frame='fk5')
#        Simbad_table = Simbad.query_region(co, radius=0.01 * u.deg)#, equinox='J2000.0')
#        if Simbad_table is not None:
#            if len(Simbad_table["MAIN_ID"]) > 0:
#                return Simbad_table
#            else:
#                return False
#        else:
#            return False
#
#    def get_SDSS(self, oid):
#        
#        # get ra, dec
#        if oid != self.oid:
#            self.get_stats(oid)
#                
#        print("http://skyserver.sdss.org/dr15/en/tools/chart/navi.aspx?ra=%f&dec=%f" % (self.meanra, self.meandec))
#
#    # get SDSS DR15 crossmatch
#    def get_SDSSDR15_galaxy(self, oid, radius):
#        
#        # get ra, dec
#        if oid != self.oid:
#            self.get_stats(oid)
...
#
#        SDSS_DR15_url = "http://skyserver.sdss.org/dr15/SkyServerWS"
#        r = requests.get(url = "%s/ConeSearch/ConeSearchService" % SDSS_DR15_url, params = params)
#
#        df = pd.read_csv(BytesIO(r.content), comment="#", index_col = "objid")
#        mask = df.type == "GALAXY"
#        if mask.sum() == 0:
#            return
#
#        # loop between nearby galaxies
#        objids = df[mask].index
#        images = []
#        for idx, objid in enumerate(objids):
#
#            ra = df[mask].loc[objid].ra
#            dec = df[mask].loc[objid].dec
#            jpgimage = "http://skyserver.sdss.org/dr15/SkyServerWS/ImgCutout/getjpeg?ra=%s&dec=10.91817&width=512&height=512&opt=OG"
#            params = {
#                "ra": "%f" % ra,
#                "dec": "%f" % dec,
#                "width": "256",
...
#            }
#
#            r = requests.get(url = "%s/ImgCutout/getjpeg" % SDSS_DR15_url, params = params)
#            
#            images.append(str(base64.b64encode(r.content).decode("utf-8")))
#
#
#        html_images = """
#        &emsp;&emsp;
...
#        for image in images:
#            html_images += """
#            <div style="float:left;width:20%%"><img src="data:image/jpeg;base64,%s"></div>
#            """ % image
#        html_images += """
#        </div>
#        """
...
#
#        
#        a = input("Please enter most likely host galaxy (%i-%i):  " % (0, len(objids) - 1))
#
#        objid = objids[int(a)]
#
#        params = {
